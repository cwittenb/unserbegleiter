...
<body><div id="app" data-core-hash="${hash}"></div><script src="/app.js"></script></body></html>\n`);
  ...
  await writeFile(path.join(outDir, "wrangler.toml"), [
    ...
    "#          SMTP_FROM ist faktisch PFLICHT: ohne ihn steht die Login-Adresse",
    "#          im Absender (und leitet den HELO-Namen ab). Gesetzt gehoert die",
    "#          Produktadresse, z. B. begleitung@raumzuzweit.de — dann muss der",
    "#          Provider diese Adresse fuer den Account freigeben UND die Domain",
    "#          SPF/DKIM tragen, sonst landet alles im Spam.",
    "#          optional SMTP_HELO: der Name im EHLO. Er MUSS ein qualifizierter",
    "#          Hostname sein (Punkt drin) — Postfix weist alles andere ab",
    "#          (reject_non_fqdn_helo_hostname, sichtbar als 504 erst bei RCPT).",
    "#          Ohne SMTP_HELO nimmt der Mailer die Domain aus SMTP_FROM, sonst",
    "#          SMTP_HOST. Nur setzen, wenn der Provider einen bestimmten Namen",
    "#          erwartet.",
    "# E-Mail-Pflicht (S45, seit S115 der Normalfall): Wer ohne bestätigte",
    "#          Adresse in der App landet, wird zuerst zur Adresse geführt — es",
    "#          braucht dafür KEINE Variable. Der Zugang haengt sonst an einem",
    "#          einzigen Cookie; geht das Geraet verloren, ist der Raum weg.",
    "#          [vars] EMAIL_PFLICHT = \"0\" ist der NOTAUS und nur fuer einen",
    "#          Fall gedacht: der Mailversand ist gestoert und die App wuerde",
    "#          sonst niemanden mehr hereinlassen. Setzen, Stoerung beheben,",
    "#          wieder entfernen — er gehoert nicht in den Dauerbetrieb.",
    "#          Ihn zu ziehen ist seit S115 nicht mehr dringend: die App macht",
    "#          nach zwei gescheiterten Sendeversuchen von selbst einen Ausgang",
    "#          auf (24 h je Geraet). Der Notaus bleibt fuer den Fall, dass die",
    "#          Stoerung laenger dauert oder gar nicht erst zum Versand kommt.",
    ...
    "# Betriebsmeldungen (S120): wrangler secret put TELEGRAM_TOKEN / TELEGRAM_CHAT",
    "#          Ohne beides bleibt der Kanal still (kein Fehler, nur nicht",
    "#          eingerichtet). Eine Stoerungsart meldet sich hoechstens einmal je",
    "#          Stunde. Bewusst NICHT per Mail: die haeufigste Stoerung ist der",
    "#          Mailversand selbst. Pruefen in der Verwaltung: \"Meldeweg pruefen\".",
