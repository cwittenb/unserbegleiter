...
function einrichtung(store, vorbelegt = {}) {
  {
    app.innerHTML = `
      ...
      <p style="color:var(--rz-leise);font-size:13px">${t("einr.umgebung", { version: CORE_VERSION })}</p>
      <div style="background:var(--rz-karte);border:1px solid var(--rz-karte-rand);border-radius:14px;padding:16px;backdrop-filter:blur(8px)">
        <label style="display:block;font-size:14px;margin:6px 0">${t("einr.nameA")}<input id="inA" style="display:block;padding:9px;border:1px solid var(--rz-feld-rand);background:var(--rz-feld);color:var(--rz-feld-ink);border-radius:9px;width:220px;font:inherit" value="Anna"></label>
        <label style="display:block;font-size:14px;margin:6px 0">${t("einr.nameB")}<input id="inB" style="display:block;padding:9px;border:1px solid var(--rz-feld-rand);background:var(--rz-feld);color:var(--rz-feld-ink);border-radius:9px;width:220px;font:inherit" value="Bernd"></label>
        <button id="btnStart" style="margin-top:8px;background:var(--rz-knopf);color:var(--rz-blase-ich-ink,#fff);border:0;border-radius:999px;padding:11px 24px;font:inherit;cursor:pointer">${t("einr.los")}</button>
        <label style="display:block;font-size:13px;margin:10px 0 0;color:var(--rz-leise)">${t("einr.sprache")}<select id="inSpr" style="font:inherit;padding:4px 8px;border:1px solid var(--rz-feld-rand);background:var(--rz-feld);color:var(--rz-feld-ink);border-radius:9px"><option value="de"${getLocale() === "de" ? " selected" : ""}>Deutsch</option><option value="en"${getLocale() === "en" ? " selected" : ""}>English</option></select></label>
      </div>`;
...
function rollenwahl(store, meta) {
  app.innerHTML = `
    ...
    <p style="color:var(--rz-leise);font-size:13px">${t("einr.wer")}</p>
    <div style="background:var(--rz-karte);border:1px solid var(--rz-karte-rand);border-radius:14px;padding:16px;backdrop-filter:blur(8px)">
      <button id="asA" style="border:1px solid var(--rz-knopf);background:transparent;color:var(--rz-knopf-ink);border-radius:999px;padding:10px 22px;font:inherit;cursor:pointer;margin-right:8px">${meta.nameA}</button>
      <button id="asB" style="border:1px solid var(--rz-knopf);background:transparent;color:var(--rz-knopf-ink);border-radius:999px;padding:10px 22px;font:inherit;cursor:pointer">${meta.nameB}</button>
      <button id="btnSelftest" style="float:right;border:1px solid var(--rz-karte-rand);background:var(--rz-karte);color:var(--rz-leise);border-radius:999px;padding:9px 16px;font:inherit;cursor:pointer">${t("einr.selbsttest")}</button>
    </div>
